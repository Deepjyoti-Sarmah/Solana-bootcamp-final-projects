...
export const mint = (args: MintArgs): TransactionInstruction => {
    const data = serialize(
        ...
        {
            id: GenerativeNftInstruction.Mint,
            pattern: args.pattern,
        }
    );

    const [generativePubkey] = pda.deriveGenerativeMetadataPDA({
        mint: args.mint,
    }, _programId);
    ...
    return new TransactionInstruction({
        ...
        keys: [
            ...
            {pubkey: generativePubkey, isSigner: false, isWritable: true},
...
export const transfer = (args: TransferArgs): TransactionInstruction => {
    const data = serialize(
        ...
        {
            id: GenerativeNftInstruction.Transfer,
        }
    );
...
export const burn = (args: BurnArgs): TransactionInstruction => {
    const data = serialize(
        ...
        {
            id: GenerativeNftInstruction.Burn,
        }
    );
...
export const getGenerativeMetadata = async (
    publicKey: PublicKey,
    commitmentOrConfig: Commitment | GetAccountInfoConfig | undefined = "processed"
): Promise<T.GenerativeMetadata | undefined> => {
    ...
    return T.decodeGenerativeMetadata(deserialize(T.GenerativeMetadataSchema, buffer.data) as Record<string, unknown>);
}
