...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("generative_nft", program_id)
			.start();
		...
		let (generative_pubkey, _) = Pubkey::find_program_address(
			&[b"generative", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(generative_pubkey, false),
		...
		let data = GenerativeNftInstruction::Mint(crate::generated::instructions::MintArgs{
			pattern,
		});
		let signers = vec![
			&fee_payer_info,
